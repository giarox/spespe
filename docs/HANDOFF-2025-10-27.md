...
- Handoff refresh: `pnpm handoff:update` captures env/access snapshots into a dated `docs/HANDOFF-*-auto.md` file and stamps `docs/SYSTEM-PROMPT.md`.
...
4. Run `pnpm handoff:update` to snapshot current state (commit the generated doc if handing over).
5. Validate Supabase connectivity (`pnpm access:check` HTTP codes).
6. Run `pnpm dev` (frontend) and optional `cd scraper && pnpm start -- --max-pages=2` for smoke test.
7. When coding, keep `.env.local` 0600; never commit secrets; run lint/build before PR.
