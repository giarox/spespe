-- Migration to improve fuzzy search using word_similarity
-- This fixes issues where short search terms failed to match long product names (e.g. "mozzarlla" vs "MOZZARELLA DI BUFALA CAMPANA DOP")
  ...
  WITH search_base AS (
    ...
    WHERE search_text IS NOT NULL
      ...
      AND (
        ...
        OR word_similarity(search_text, coalesce(p.product_name, '')) >= 0.3
        OR word_similarity(search_text, coalesce(p.brand, '')) >= 0.3
      )
  ),
  ranked AS (
