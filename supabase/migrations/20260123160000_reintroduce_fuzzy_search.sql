...
AS $function$
  WITH search_base AS (
    SELECT p.*, 
           ...
           greatest(
             similarity(search_text, coalesce(p.product_name, '')),
             similarity(search_text, coalesce(p.brand, '')),
             similarity(search_text, coalesce(p.description, ''))
           ) AS fuzzy_score
    FROM products p
  ...
  scored AS (
    ...
    WHERE p.product_name ILIKE search_text || '%'
       ...
       OR p.fuzzy_score > 0.5
  ),
  ranked AS (
    SELECT *, greatest(search_rank, prefix_score, fuzzy_score) AS score,
           row_number() OVER (
             PARTITION BY chain_id, flyer_id, lower(product_name)
             ORDER BY greatest(search_rank, prefix_score, fuzzy_score) DESC,
